      ...
      <Card className="h-auto">
        <CardHeader className={compact ? "pb-2 py-2" : "pb-3"}>
          <h3 className={compact ? "text-xs font-medium flex items-center gap-1" : "text-sm font-medium flex items-center gap-2"}>
            <MessageSquare className={compact ? "h-3 w-3 text-gray-500" : "h-4 w-4 text-gray-500"} />
            Analysis Context
          </h3>
        </CardHeader>
    ...
    <Card className="h-auto">
      ...
      >
        <div className="flex items-center justify-between">
          <h3 className={compact ? "text-xs font-medium flex items-center gap-1" : "text-sm font-medium flex items-center gap-2"}>
            <Sparkles className={compact ? "h-3 w-3 text-purple-500" : "h-4 w-4 text-purple-500"} />
            Analysis Context
      ...
      {isCardExpanded && (
        <CardContent className={compact ? "space-y-2 pt-0" : "space-y-3"}>
          {conversations.slice(0, compact ? 2 : 5).map((conversation, index) => {
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  <div className="flex items-start justify-between mb-2">
                    <div className="flex-1">
                      ...
                      {conversation.relevance_score && (
                        <Badge className={`text-xs ${getRelevanceColor(conversation.relevance_score)}`}>
                          {Math.round(conversation.relevance_score * 100)}% relevant
                        </Badge>
                      )}
                    </div>
                  ...
                  {/* Key moments badges */}
                  {!isExpanded && conversation.key_moments && conversation.key_moments.length > 0 && (
                    <div className="flex flex-wrap gap-1 mt-2">
                      {conversation.key_moments.slice(0, 3).map((moment, i) => (
                ...
                {isExpanded && (
                  <div className="border-t px-3 py-2 space-y-2 bg-gray-50">
                    ...
                    {conversation.key_moments && conversation.key_moments.length > 0 && (
                      <div className="mt-3 pt-2 border-t">
                        <h4 className="text-xs font-medium text-gray-700 mb-1">Key Moments:</h4>
          ...
          {conversations.length > (compact ? 2 : 5) && (
            <p className="text-xs text-gray-500 text-center">
              +{conversations.length - (compact ? 2 : 5)} more conversation segments
            </p>
          )}
        </CardContent>
      )}
