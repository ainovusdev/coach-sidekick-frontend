...
/**
 * One create form, two doors. An admin reaches it from the admin console; a
 * coach from their own sandbox dashboard. Whoever submits becomes the
 * sandbox's account executive (the API does that), and lands on the cockpit
 * their chrome knows.
 */
export function NewSandboxForm({ audience }: { audience: 'admin' | 'coach' }) {
  const router = useRouter()
  const createSandbox = useCreateSandbox()
  const listHref = audience === 'admin' ? '/admin/sandboxes' : '/sandboxes'
  const sandboxHref = (id: string) =>
    audience === 'admin' ? `/admin/sandboxes/${id}` : `/sandboxes/${id}`
  // Organisation suggestions and the duplicate-name check read whichever list
  // the caller may see: the whole portfolio for an admin, their own for a coach.
  const portfolio = useSandboxes({ include_ended: true }, audience === 'admin')
  const mine = useMySandboxes(audience === 'coach')
  const existing = audience === 'admin' ? portfolio.data : mine.data
  ...
  const onSubmit = handleSubmit(async values => {
    ...
    router.push(sandboxHref(created.sandbox.id))
  })
