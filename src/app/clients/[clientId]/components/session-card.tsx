...
import { ChevronRight, FileText, Brain, Clock, Calendar, Trash2, MoreVertical } from 'lucide-react'
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
        ...
        <div className="space-y-4">
          ...
          {session.key_topics && session.key_topics.length > 0 && (
            <div className="flex flex-wrap gap-1.5">
              {session.key_topics.slice(0, 5).map((topic: string, index: number) => (
                <Badge 
                  key={index} 
                  variant="secondary" 
          ...
          {session.action_items && session.action_items.length > 0 && (
            <div className="space-y-1.5">
              ...
              <ul className="space-y-1">
                {session.action_items.slice(0, 2).map((item: string, index: number) => (
                ...
                {session.action_items.length > 2 && (
                  <li className="text-sm text-gray-500 italic">
                    +{session.action_items.length - 2} more items
                  </li>
                )}
              </ul>
            </div>
